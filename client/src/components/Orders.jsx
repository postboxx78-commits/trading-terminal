...
export default function Orders() {
  ...
  const getStatusIcon = (status) => {
    ...
    if (s.includes('complete') || s.includes('executed')) return <CheckCircle className="w-3 h-3 text-[#00D09C]" />;
    if (s.includes('reject') || s.includes('failed')) return <XCircle className="w-3 h-3 text-[#FF4D4D]" />;
  ...
  if (error) return <div className="p-4 text-[#FF4D4D] text-center">{error}<button onClick={fetchOrderBook} className="ml-2 text-gray-400">Retry</button></div>;

  return (
    <div className="h-full flex flex-col bg-[#13151A]">
      <div className="px-3 py-2 border-b border-[#2C2F36] bg-[#111217] flex flex-wrap items-center justify-between gap-2">
        <div className="flex gap-1">
          <button onClick={() => setActiveTab("orders")} className={`px-2 py-1 text-[10px] rounded ${activeTab === "orders" ? 'bg-[#3B82F6] text-white' : 'text-gray-400'}`}>Orders ({orders.length})</button>
          <button onClick={() => setActiveTab("trades")} className={`px-2 py-1 text-[10px] rounded ${activeTab === "trades" ? 'bg-[#3B82F6] text-white' : 'text-gray-400'}`}>Trades ({trades.length})</button>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={() => setAutoRefresh(!autoRefresh)} className={`text-[9px] px-1.5 py-0.5 rounded ${autoRefresh ? 'bg-[#00D09C]/20 text-[#00D09C]' : 'bg-[#1A1C23] text-gray-400'}`}>Auto</button>
        ...
        {activeTab === "orders" && (
          <select value={filter} onChange={e => setFilter(e.target.value)} className="text-[10px] bg-[#1A1C23] border border-[#2C2F36] rounded px-1 py-0.5 text-gray-300">
        ...
        {activeTab === "orders" ? (
          filteredOrders.length === 0 ? <div className="p-4 text-center text-gray-500">No orders</div> : (
            <table className="w-full text-[10px]">
              <thead className="sticky top-0 bg-[#13151A] text-gray-400"><tr><th className="p-2 text-left">ID</th><th className="p-2 text-left">Symbol</th><th className="p-2 text-left">Type</th><th className="p-2 text-right">Qty</th><th className="p-2 text-right">Price</th><th className="p-2 text-left">Status</th><th className="p-2 text-left">Actions</th></tr></thead>
              <tbody>
                {filteredOrders.map(o => (
                  <tr key={o.orderId} className="border-t border-[#2C2F36] hover:bg-[#1A1C23]">
                    <td className="p-2 font-mono text-gray-300">{o.orderId.slice(-6)}</td>
                    <td className="p-2 font-medium text-gray-200">{o.symbol}</td>
                    <td className="p-2"><span className={o.transactionType === "BUY" ? "text-[#00D09C]" : "text-[#FF4D4D]"}>{o.transactionType}</span><div className="text-[8px] text-gray-500">{o.orderType}</div></td>
          ...
          trades.length === 0 ? <div className="p-4 text-center text-gray-500">No trades</div> : (
            <table className="w-full text-[10px]">
              <thead className="sticky top-0 bg-[#13151A] text-gray-400"><tr><th className="p-2 text-left">Order ID</th><th className="p-2 text-left">Symbol</th><th className="p-2 text-left">Type</th><th className="p-2 text-right">Qty</th><th className="p-2 text-right">Price</th><th className="p-2 text-left">Time</th></tr></thead>
              <tbody>
                {trades.map(t => (
                  <tr key={t.exchangeOrderId} className="border-t border-[#2C2F36] hover:bg-[#1A1C23]">
                    <td className="p-2 font-mono text-gray-300">{t.orderId?.slice(-6)}</td>
                    <td className="p-2 text-gray-200">{t.symbol}</td>
                    <td className="p-2"><span className={t.transactionType === "BUY" ? "text-[#00D09C]" : "text-[#FF4D4D]"}>{t.transactionType}</span></td>
      ...
      {lastUpdated && <div className="p-1 border-t border-[#2C2F36] text-[8px] text-gray-500 text-right">Updated: {lastUpdated.toLocaleTimeString()}</div>}
      {/* History Modal */}
      {showHistoryModal && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-[#13151A] rounded-xl w-full max-w-md border border-[#2C2F36] p-3">
            ...
            <div className="max-h-96 overflow-auto space-y-2">
              {orderHistory.map((h, i) => <div key={i} className="bg-[#1A1C23] p-2 rounded text-[10px]">{h.status} - {h.filledDateTime}<div className="text-gray-400">Qty: {h.quantity} @ ₹{h.price}</div></div>)}
            </div>
          </div>
        </div>
      )}
      ...
      {showModifyModal && selectedOrder && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-[#13151A] rounded-xl w-full max-w-sm border border-[#2C2F36] p-3">
            ...
            <div className="mb-2"><label className="text-[10px] text-gray-400">Quantity</label><input type="number" value={modifyData.quantity} onChange={e => setModifyData({...modifyData, quantity: parseInt(e.target.value)})} className="w-full px-2 py-1 text-xs bg-[#1A1C23] border border-[#2C2F36] rounded" /></div>
            <div className="mb-2"><label className="text-[10px] text-gray-400">Price (₹)</label><input type="number" value={modifyData.price} onChange={e => setModifyData({...modifyData, price: e.target.value})} className="w-full px-2 py-1 text-xs bg-[#1A1C23] border border-[#2C2F36] rounded" /></div>
            <div className="mb-2"><label className="text-[10px] text-gray-400">Validity</label><select value={modifyData.validity} onChange={e => setModifyData({...modifyData, validity: e.target.value})} className="w-full px-2 py-1 text-xs bg-[#1A1C23] border border-[#2C2F36] rounded"><option>DAY</option><option>IOC</option></select></div>
            <button onClick={modifyOrder} className="w-full bg-[#3B82F6] py-1.5 rounded text-xs font-medium">Modify</button>
          </div>
        </div>
      )}
    </div>
