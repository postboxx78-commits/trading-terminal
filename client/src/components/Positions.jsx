  ...
  if (error) {
    return (
      <div className="p-4 text-center">
        ...
        <button onClick={fetchPositions} className="mt-2 text-xs text-gray-500">Retry</button>
      </div>
    );
  }
    ...
    <div className="h-full flex flex-col bg-white">
      <div className="px-3 py-2 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
        <div className="flex items-center gap-2">
          <h3 className="text-xs font-semibold text-gray-500">Positions</h3>
          <span className="text-[9px] text-gray-500 bg-gray-200 px-1.5 py-0.5 rounded">{positions.length}</span>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={() => setAutoRefresh(!autoRefresh)} className={`text-[9px] px-1.5 py-0.5 rounded ${autoRefresh ? 'bg-green-100 text-green-700' : 'bg-gray-200 text-gray-600'}`}>
            {autoRefresh ? 'Auto' : 'Manual'}
          </button>
          <button onClick={fetchPositions} disabled={loading} className="text-gray-400 hover:text-gray-600">
      ...
      <div className="flex-1 overflow-auto">
        {loading && !positions.length ? (
          <div className="flex justify-center p-6"><div className="animate-spin rounded-full h-6 w-6 border-2 border-blue-600 border-t-transparent" /></div>
        ) : positions.length === 0 ? (
          <div className="flex flex-col items-center p-6 text-gray-400"><Minus className="w-8 h-8 mb-2" /><p className="text-xs">No open positions</p></div>
        ) : (
          <table className="w-full text-xs">
            ...
            <tbody>
              {positions.map(pos => (
                <tr key={pos.tradingSymbol} className="border-t border-gray-100 hover:bg-gray-50">
                  <td className="p-2">
                    <div className="font-medium text-gray-800">{pos.symbol}</div>
                    <div className="text-[9px] text-gray-400">{pos.product} • {pos.exchange}</div>
                  </td>
                  <td className={`p-2 text-right font-medium ${pos.quantity > 0 ? 'text-green-600' : 'text-red-600'}`}>{pos.quantity}</td>
                  <td className="p-2 text-right text-gray-600">{formatCurrency(pos.positionType === "LONG" ? pos.averageBuyPrice : pos.averageSellPrice)}</td>
                  <td className="p-2 text-right font-medium text-gray-800">{formatCurrency(pos.lastPrice)}</td>
                  <td className={`p-2 text-right font-medium ${pos.pnl >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                  ...
                  <td className="p-2 text-right">
                    <button onClick={() => squareOff(pos)} disabled={squareOffLoading === pos.tradingSymbol} className="text-[9px] px-2 py-0.5 rounded bg-red-50 text-red-600 hover:bg-red-100">
      ...
      {positions.length > 0 && (
        <div className="p-2 border-t border-gray-100 bg-gray-50 text-[9px]">
          <div className="flex justify-between"><span className="text-gray-500">Investment:</span><span className="font-medium">{formatCurrency(totalInvestment)}</span></div>
          <div className="flex justify-between mt-1"><span className="text-gray-500">Total P&L:</span><span className={`font-medium ${totalPnl >= 0 ? 'text-green-600' : 'text-red-600'}`}>{formatCurrency(totalPnl)}</span></div>
          {lastUpdated && <div className="text-right text-gray-400 mt-1">Updated: {lastUpdated.toLocaleTimeString()}</div>}
        </div>
      )}
    </div>
  );
